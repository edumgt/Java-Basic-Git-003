...
#### 1주차 : WSL 환경 구축, VScode 설치 및 각종 확장팩 설정, AI 에이전트 연결
#### 1주차 : Git , GitHub , GH 등 플랫폼 모듈 설치
#### 2주차 : Docker 설치 및 Docker Hub 가입
#### 3주차 : AWS 가입 및 AWS CLI 설치
#### 4주차 : AWS 기반 Lambda, API GW, Site 배포


### 수업 50분중 쉬는 시간 포함 30분 이상의 시간은 각자 이어폰, 헤드셋등을 이용해 시청각
### 장기과정 진행으로 인한 미래에 대한 불안감 해소가 중요하며, 기술스택, 퀀트 등의 단어를 민간 개발 수급 플랫폼 wishket 등에서 검색해 보는걸 추천 합니다.
### 개인 별로 PC 사용이 능숙하지 않은 경우 구글의 antigravity 를 설치해서 AI 지원을 받습니다.
...
## 본 과정에서는 종목 추천은 금지 입니다. 강사대 학생 및 학생 간 종목 추천 금지 입니다.
## 본 과정은 금융관련 자격증 취득과는 무관하며, 해당 정보는 해당 협회등의 공인 받은 교육기관만 가능하므로, 여기서는 다루지 않습니다.
## 본 과정은 IT 종사자가 금융관련업무를 분석하여 프로젝트를 수행 하는 과정 입니다. 따라서, 팀프로젝트에서 PO, PM, SA 의 RnR 만 있으며, 코더는 AI 에이전트 입니다.

![alt text](images/readme-image-13.png)


## 본 과정의 강사는 자산운용 자격증이 있거나 금융권 종사자가 아닌 IT 엔지니어로서 금융 분야의 업무를
## 이해하고, 퀀트 시스템 개발을 위한 IT 기술을 전수하는 강사로서, KDT - NCS 의 강사 자격은 
## 워크넷의 경우 법인 운용 경력증명으로 가능한 상태임을 공유합니다.

## 이 과정의 3 분야

## 컴공 전공 또는 관련 분야 종사자 - 주식 투자의 기본 이해 및 개발을 위한 파트
### 금융 관련 상식을 넓히기 위한 각종 뉴스, 유튜브, AI 를 통한 검색을 요구 합니다.


## 금융 전공 또는 관련 분야 종사자 - Product Owner 로서 프로젝트 리드, IT 개발 기술 연습
### IT 기술을 익혀야 필요한 프로그래밍이 가능하므로, AI 에이전트를 이용한 시스템 사용 연습을 요구 합니다.

## 기타 본인 지향 하는 바가 ML/DL 연구 분야 라면 - Python 및 ML/DL 등의 적용
### Python, 각종 라이브러리, JupyterNoteBook 등을 통한 언어 중심의 연습을 요구 합니다.
...
| **java-lab** | Java 초급 입문 | Java, 전자정부 표준프레임워크 |
...
* **특징 및 장점:**
    * **편의성:** `ollama run llama3` 명령어 하나면 모델 다운로드부터 API 서버 구동까지 끝납니다.
...
> **각종 자료·논문·보고서를 업로드하면 그 안의 내용만 근거로 답변하는 "출처 기반(source-grounded)" AI 노트 도구입니다.**

### 개요
NotebookLM은 업로드한 문서(소스)만을 근거로 요약·질의응답·오디오 요약(팟캐스트)·마인드맵 등을 생성합니다. 일반 챗봇과 달리 답변에 항상 **출처 인용(citation)**이 붙어 어느 문서·페이지에서 나온 내용인지 추적할 수 있어, 각종 자료 분석이나 논문 리뷰에 특히 유용합니다.
...
| Audio Overview | 소스 내용을 두 명의 AI 진행자가 대화하는 팟캐스트 형태 오디오로 자동 생성(복습·이동 중 청취용) |
...
```
# 소스 업로드 후 질의 예시
"이 자료에서 3장의 핵심 개념 3가지를 요약해줘 (출처 포함)"
...
### 학습 포인트
- 각종 자료를 올려 **수업 준비용 FAQ/타임라인 자동 생성**
